...
def run_module_benchmark(
    ...
    """Run module-level benchmarking using model_name approach."""
    if unified_config.model_config is None:
        raise ValueError("model_config is required for module benchmarking")
    ...
    # Create model config and generate model using existing infrastructure
    ...
    # Generate the model using the unified model generation approach
    module = model_config.generate_model(
        tables=tables,
        weighted_tables=weighted_tables,
        dense_device=torch.device("meta"),
    )
